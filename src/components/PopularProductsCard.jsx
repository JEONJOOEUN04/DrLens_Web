...
import { popularProducts } from '../api/products'

function PopularProductsCard() {
  const { data, isLoading, isError } = useQuery({
    queryKey: ['popular-products-top10'],
    queryFn: () => popularProducts({ page: 1, size: 10 }),
    ...
    <div className="bg-card border border-line rounded-2xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="flex items-center gap-1 text-[11px] font-bold text-danger bg-danger/10 px-2.5 py-1 rounded-full">
          <Heart size={11} className="fill-danger" /> Hot
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <ul className="space-y-2.5">
          {items.map((p, idx) => (
            <li
              key={p.product_id}
              className="flex items-center gap-3 py-1.5 rounded-lg hover:bg-primary-light/30 px-2 -mx-2 transition"
            >
              <span
                className={`w-7 text-center text-[13px] font-extrabold shrink-0 ${
                  idx < 3 ? 'text-primary' : 'text-text-sub'
                }`}
              >
                {idx + 1}
              </span>
              ...
              <div className="text-right shrink-0">
                {p.like_count != null && (
                  <span className="flex items-center gap-1 text-[11px] font-bold text-danger justify-end">
                    <Heart size={11} className="fill-danger" /> {p.like_count}
                  </span>
                )}
                {p.price && (
